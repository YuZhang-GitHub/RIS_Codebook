...
if __name__ == '__main__':
    ...
    options = {
        'gpu_idx': 3,
        'num_ant_all': 64,
        'num_block_ph': 2,  # equal to number of sub-arrays
        'num_NNs': 8,  # number of beams
        'num_bits': 4
    }

    LIS_id = os.path.abspath('..')[-1]
    options['path'] = '../S1/LIS_' + LIS_id + '_grid1201_1-1400_80.mat'
    ...
    for ii in range(num_sub_array):
        for kk in range(num_beam):
            fname = '../S' + str(ii + 1) + '/beams/beams_' + str(kk) + '_max.txt'
    ...
    label_load = scio.loadmat('../S1/Distributed_LIS_label_1201_1-1400_80-' + str(options['num_NNs']) + '.mat')['labels']
    ch_group = []
    beams_group = []
