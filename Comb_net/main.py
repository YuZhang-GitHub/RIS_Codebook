...
if __name__ == '__main__':
    ...
    options = {
        'gpu_idx': 3,
        'num_ant_all': 256,
        'num_block_ph': 4,  # equal to number of sub-LISs
    ...
    num_ant_sub_LIS = int(options['num_ant_all'] / options['num_block_ph'])
    num_sub_LIS = options['num_block_ph']
    num_beam = options['num_NNs']
    phase_pattern = np.zeros((num_sub_LIS, num_ant_sub_LIS, num_beam))

    for ii in range(num_sub_LIS):
    ...
    # options['sub_phase_pattern'] = np.random.randn(8, 32)  # test only, data flow
